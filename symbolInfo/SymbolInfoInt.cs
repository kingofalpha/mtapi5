...
namespace test.symbolInfo
{
  ...
  public  class SymbolInfoInt
    {
        private MtApi5Client mt { get; }
        public SymbolInfoInt(string symbolName,MtApi5Client mt)
        {
            this.mt = mt;
        ...
        public ENUM_SYMBOL_SWAP_MODE SWAP_MODE { get; private set; } //交易计算模式 	ENUM_SYMBOL_SWAP_MODE
        public ENUM_SYMBOL_SWAP_MODE SWAP_ROLLOVER3DAYS { get; private set; }//	 ENUM_DAY_OF_WEEK 日翻滚收费
        public bool MARGIN_HEDGED_USE_LEG { get; private set; } // 计算锁仓预付款只收取单边交易手数大的（买入或卖出）
        public ENUM_SYMBOL_ORDER_GTC_MODE EXPIRATION_MODE { get; private set; } //	 到期模式 允许命令标志
        ...
        private void RefreshFix()
        {
            ...
            SWAP_MODE = (ENUM_SYMBOL_SWAP_MODE)getInt(ENUM_SYMBOL_INFO_INTEGER.SYMBOL_SWAP_MODE);
            SWAP_ROLLOVER3DAYS = (ENUM_SYMBOL_SWAP_MODE) getInt(ENUM_SYMBOL_INFO_INTEGER.SYMBOL_SWAP_ROLLOVER3DAYS);
            MARGIN_HEDGED_USE_LEG = getInt(ENUM_SYMBOL_INFO_INTEGER.SYMBOL_MARGIN_HEDGED_USE_LEG) == 1;
            EXPIRATION_MODE =(ENUM_SYMBOL_ORDER_GTC_MODE)getInt(ENUM_SYMBOL_INFO_INTEGER.SYMBOL_EXPIRATION_MODE);
        ...
        public async void test()
        {
            ...
            long a =await SESSION_DEALS;
            Console.Write(a.ToString());
            Console.WriteLine("-----1");
         
            
            a = await SESSION_BUY_ORDERS;
            Console.Write(a.ToString());
            Console.WriteLine("-----2");
            a = await SESSION_SELL_ORDERS;
            Console.Write(a.ToString() );
            Console.WriteLine("-----3");
            a = await SYMBOL_VOLUME;
            Console.Write(a.ToString() );
            Console.WriteLine("-----4");
            a = await VOLUMEHIGH;
            Console.WriteLine(a.ToString() + "-----5");
            a = await VOLUMELOW;
            Console.WriteLine(a.ToString() + "-----6");
            a = await TIME;
            Console.WriteLine(a.ToString() + "-----7");
            a = await TICKS_BOOKDEPTH;
            Console.WriteLine(a.ToString() + "-----8");
            a = await TRADE_STOPS_LEVEL;
            Console.WriteLine(a.ToString() + "-----9");
            a = await TRADE_FREEZE_LEVEL;
            Console.WriteLine(a.ToString() + "-----10");
            Console.WriteLine(new DateTimeOffset(DateTime.UtcNow).ToUnixTimeMilliseconds() - now);
            Console.WriteLine("\r\n");


        }

        private async Task<long> getIntx(ENUM_SYMBOL_INFO_INTEGER arg)
        {
        ...
        public Task<long> SESSION_DEALS { get { return getIntx(ENUM_SYMBOL_INFO_INTEGER.SYMBOL_SESSION_DEALS); } private set { } }//当前时期的交易数量
        public Task<long> SESSION_BUY_ORDERS { get { return getIntx(ENUM_SYMBOL_INFO_INTEGER.SYMBOL_SESSION_BUY_ORDERS); } }//	当时的买入订单数量
        public Task<long> SESSION_SELL_ORDERS { get { return getIntx(ENUM_SYMBOL_INFO_INTEGER.SYMBOL_SESSION_SELL_ORDERS); } } //当时的卖出订单数量
        public Task<long> SYMBOL_VOLUME { get { return getIntx(ENUM_SYMBOL_INFO_INTEGER.SYMBOL_VOLUME); } }//最后订单成交量
        public Task<long> VOLUMEHIGH { get { return getIntx(ENUM_SYMBOL_INFO_INTEGER.SYMBOL_VOLUMEHIGH); } }//当天最大订单

        public Task<long> VOLUMELOW { get { return getIntx(ENUM_SYMBOL_INFO_INTEGER.SYMBOL_VOLUMELOW); } } //当天最小订单
        public Task<long> TIME { get { return getIntx(ENUM_SYMBOL_INFO_INTEGER.SYMBOL_TIME); } } // 最后报价时间
        public Task<long> TICKS_BOOKDEPTH {  get { return  getIntx(ENUM_SYMBOL_INFO_INTEGER.SYMBOL_TICKS_BOOKDEPTH); } } // 显示在市场深度要求中的最大数量，交易品种无队列要求，值是0

        public Task<long> TRADE_STOPS_LEVEL { get { return getIntx(ENUM_SYMBOL_INFO_INTEGER.SYMBOL_TRADE_STOPS_LEVEL); } } // 止蚀盘当前收盘价格的最小空间
        public Task<long> TRADE_FREEZE_LEVEL { get { return getIntx(ENUM_SYMBOL_INFO_INTEGER.SYMBOL_TRADE_FREEZE_LEVEL); } } //凝结交易操作的距离
